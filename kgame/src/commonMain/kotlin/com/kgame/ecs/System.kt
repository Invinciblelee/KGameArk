...
abstract class IntervalSystem(
     ...
     */
    val world: World = World.requireCurrentWorld()
     ...
     */
    open fun onRender(drawScope: DrawScope) = Unit
 ...
 *
 * [Manual] means that sorting must be called programmatically by setting [IteratingSystem.requiresSort] to true.
 ...
 * @param comparator an optional [com.kgame.ecs.collection.EntityComparator] that is used to sort [entities][Entity].
...
abstract class IteratingSystem(
     ...
     */
    var requiresSort = sortingType == Automatic && comparator != EMPTY_COMPARATOR

    /**
     * If [requiresSort] is true then [entities][Entity] are sorted using the [comparator] before calling [onTickEntity].
     */
    open fun onSort() {
        if (requiresSort) {
            requiresSort = sortingType == Automatic
    ...
    override fun onRender(drawScope: DrawScope) {
        family.forEach { onRenderEntity(it, drawScope) }
    }

    /**
     * Function that contains the rendering logic for each [entity][Entity] of the system.
     */
    open fun onRenderEntity(entity: Entity, drawScope: DrawScope) = Unit
    ...
    companion object {
        val EMPTY_COMPARATOR = Comparator<Entity> { _, _ -> 0 }
    }
}
